...
const Login = () => {
    ...
    const handleSubmit= async(e)=>{
            ...
            if(email && password)
            {
                const fetchData=await fetch(`${import.meta.env.REACT_APP_SERVER_DOMAIN}/login`,{
                    method:"POST",
                    headers:{
  ...
  return (
    ...
    <div className="w-full max-w-sm bg-white m-auto flex items-center flex-col p-4">
        ...
        <button 
        className="w-full max-w-[150px] m-auto w-full bg-red-500 hover:bg-red-600 cursor-pointer text-white text-xl font-mediuum text-center py-1 rounded-full mt-4">
           LogIn
        </button>
        </form>
        <p 
        className='text-left'>Don't Have Account ? <Link to={"/signup"} className='text-red-500 underline'>Sign Up</Link></p>
    </div>
</div>
  )
}
